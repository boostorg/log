 ...
 * it can produce and an optional truncation marker string, which will get appended if the limit is exceeded. Note that
 * the marker length is not accounted for by the limit.
 ...
 *
 * \note The \a overflow_marker length is not included in the \a max_size limit.
 */
template< typename CharT >
